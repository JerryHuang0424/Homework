        ...
        for i in range(0, SearchAgents_no):
            ...
            if kk[i] > 0.5:
                ...
                if dim <= SearchAgents_no/5:
                    ...
                    Newpos[i, params[0]] = pos[i, params[0]] + (global_best[params[0]] - pos[i, params[1]]) * (
                            r1 + 1) * math.sin(r2 * 2 * math.pi)
                    Newpos[i, params[1]] = pos[i, params[1]] + (global_best[params[0]] - pos[i, params[1]]) * (
                            r1 + 1) * math.cos(r2 * 2 * math.pi)
                else:
                    params = numpy.arange(dim)
                    numpy.random.shuffle(params)
                    for j in range(round(dim/2)):
                        Newpos[i, 2*j-1] = pos[i, params[2*j-1]] + (global_best[params[0]] - pos[i, params[2*j-1]]) * (
                                r1 + 1) * math.sin(r2 * 2 * math.pi)
                        Newpos[i, 2*j] = pos[i, params[2*j]] + (global_best[params[0]] - pos[i, params[2*j]]) * (
                                r1 + 1) * math.cos(r2 * 2 * math.pi)
            else:
                # 开发阶段--捕食
                r3 = random.random()
