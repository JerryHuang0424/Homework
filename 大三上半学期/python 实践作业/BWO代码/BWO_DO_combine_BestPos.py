                    ...
                    if random.uniform(0,1) < 1.5:
                        if dim <= SearchAgents_no / 5:
                            ...
                            Newpos[i, params[1]] = pos[i, params[1]] + alpha * V_x * V_y * ln_y * (
                                        global_best[params[0]] - pos[i, params[1]]) * (r1 + 1) * math.cos(r2 * 2 * math.pi)
                        else:
                            params = numpy.arange(dim)
                            numpy.random.shuffle(params)
                            for j in range(round(dim/2)):
                                Newpos[i, 2*j-1] = pos[i, params[2*j-1]] + alpha * V_x * V_y * ln_y * (global_best[params[2*j-1]]-pos[2*j-1])
                                Newpos[i, 2*j] = pos[i, params[2*j]] + alpha * V_x * V_y * ln_y * (global_best[params[2*j]]-pos[2*j])
                    else:
                        if dim <= SearchAgents_no / 5:
                            indices = numpy.arange(dim)
                            numpy.random.shuffle(indices)
