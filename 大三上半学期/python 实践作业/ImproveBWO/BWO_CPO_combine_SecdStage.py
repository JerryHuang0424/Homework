        ...
        for i in range(0, SearchAgents_no):
            ...
            if kk[i] > 0.5:
                ...
                if dim <= SearchAgents_no / 5:
                    ...
                    for j in range(round(dim / 2)):
                        Newpos[i, 2 * j - 1] = pos[i, params[2 * j - 1]] + (
                                    pos[RJ, params[0]] - pos[i, params[2 * j - 1]]) * (
                                                       r1 + 1) * math.sin(r2 * 2 * math.pi)
                ...
                if numpy.random.rand() < 0.8:
                    ...
                    S = S * Yt * St
                    Newpos[i, :] = (1 - U1) * pos[i, :] + U1 * (pos[rand_index1, :] + St * (pos[rand_index2, :] - pos[rand_index1, :]) - S)
                else:
                    Mt = numpy.exp(fitness[i] / (numpy.sum(fitness) + numpy.finfo(float).eps))
                    Vtp = pos[rand_index1, :]
                    Ft = numpy.random.rand(dim) * (Mt * (-pos[i, :] + Vtp))
                    S = S * Yt * Ft
                    Newpos[i, :] = (pos[i, :] + (0.2 * (1 - numpy.random.rand()) + numpy.random.rand()) * (U2 * pos[i, :] - pos[i, :])) - S
