...
# Population Sorting returns the fitness and order after sorting
def SortFitness(Fit):
...
# Adjust the population position based on the sorting results Return the adjusted population matrix
...
def BWO(objf, lb, ub, dim, SearchAgents_no, Mapos_iter):
    ...
    fitness, sortIndepos = SortFitness(fitness)  # sort to the fitness
    pos = SortPosition(pos, sortIndepos)  # Population sorting
    fvalbest = copy.copy(fitness[0])  # 记录最优适应度值record the best fitness
