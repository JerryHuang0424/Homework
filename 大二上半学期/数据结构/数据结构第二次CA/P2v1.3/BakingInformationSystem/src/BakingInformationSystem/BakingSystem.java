...
public class BakingSystem {
    ...
    public static String listAllRIngredientByCalories() {
    ...
    public static List<BakedGood> searchBakedGood(String content, String parameter) {
        switch (parameter) {
            ...
            case "description" -> {
                List<BakedGood> list = new List<>();
                for (int i = 0; i < bakedGoodLists.size(); i++) {
                    if (computeSimilarity(bakedGoodLists.getDataByIndex(i).getDescription(), content) >= 60) {
    ...
    public static List<Ingredient> searchIngredient(String content, String parameter) {
        switch (parameter) {
            ...
            case "description" -> {
                List<Ingredient> list = new List<>();
                for (int i = 0; i < ingredientList.size(); i++) {
                    if (computeSimilarity(ingredientList.getDataByIndex(i).getDescription(), content) >= 60) {
            ...
            case "calories" -> {
                List<Ingredient> list = new List<>();
                for (int i = 0; i < ingredientList.size(); i++) {
                    if ((abs((ingredientList.getDataByIndex(i).getCalories()-Double.parseDouble(content))/Double.parseDouble(content)))<10){
                        list.addLast(ingredientList.getDataByIndex(i));
                    }
                }
    ...
    public static List<Recipe> searchRecipe(String content, String parameter) {
        switch (parameter) {
            ...
            case "calories" -> {
                List<Recipe> list = new List<>();
                for (int i = 0; i < recipeList.size(); i++) {
                    if ((abs((recipeList.getDataByIndex(i).getTotalCalories()-Double.parseDouble(content))/Double.parseDouble(content)))<10){
